...
//                requestCode, intent, FLAG);
     ...
     * @param content
     */
    public void notify_mailbox2(PendingIntent pendingIntent, int smallIcon, int largeIcon, String largeIcon2, ArrayList<String> messageList,
                                String ticker, String title, String content, boolean sound, boolean vibrate, boolean lights) {
         ...
         **/
        if (!PublicUtils.isBackground()) {
            return;
        }

        Bitmap bitmap = null;
        if (!TextUtils.isEmpty(largeIcon2)) {
            largeIcon2 = largeIcon2.contains("http") ? largeIcon2 : "http:" + largeIcon2;
            bitmap = ImageCacheUtils.getImageCache(largeIcon2, 100);
        } else {
            bitmap = BitmapFactory.decodeResource(mContext.getResources(), largeIcon);
        }
        cBuilder.setLargeIcon(bitmap);
